# importando bibliotecas para execução com Flask e disponibilizar monitoração prometheus com bb-flask-monitor
...
#nome da aplicação que aparece na monitoração
...
def check_db():
    try:
        response = req.get("http://localhost:5001/database")
        app.logger.info(response)
        return response.status_code < 400
    except:
        return 0  

watch_dependencies("database", check_db, app=app, time_execution=500)
